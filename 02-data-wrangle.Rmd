...
- I also mapped the results in R in the next workbook.
...
This shows there are no tracts with grocery store further away than 10 miles. This means we only really need LATracts_half, LATracts1 since no tracts have anything further.
...
## Build Food Access columns

### Reorder LILA columns for clarity
...
How to rate the tracts. Remember we already checked the 10+, 20+ for rural tracts.

- 00 -- Not a food desert. THIS SEEMS OFF. ESPECIALLY WITH LILA.
- 10 -- Supermarket more than 1/2 mile away (but within a mile?)
- 11 -- Supermarket more than 1 mile away.
...
fa_travis_lila <- fa_travis_lila %>% 
  mutate(Limited_Income_Limited_Access = 
            case_when(
              (LILATracts_halfAnd10 == 0 & LILATracts_1And10 == 0) ~ "Good access or rural",
              (LILATracts_halfAnd10 == 1 & LILATracts_1And10 == 0) ~ "Between 1/2 and 1 mile",
              (LILATracts_halfAnd10 == 1 & LILATracts_1And10 == 1) ~ "More than 1 mile"
            )
        )
...
fa_travis_la <- fa_travis_la %>% 
  mutate(Limited_Access = 
            case_when(
              (LATracts_half == 0 & LATracts1 == 0) ~ "Good access or rural",
              (LATracts_half == 1 & LATracts1 == 0) ~ "Between 1/2 and 1 mile",
              (LATracts_half == 1 & LATracts1 == 1) ~ "More than 1 mile"
            )
        )
...
# join the dataframes
tfa <- left_join(fa_travis_lila, fa_travis_la, by="CensusTract")

# peek at names to check join
tfa %>% names()

```


### Set the food access columns as factors

This sets these columns as factors and sets the level from best access to worst. This will be useful later with mapping in R.

```{r}
tfa <- tfa %>% 
  mutate(
    Limited_Income_Limited_Access = Limited_Income_Limited_Access %>%
      as_factor() %>% 
      fct_relevel(
        "Good access or rural",
...
We'll select only the columns we need in the final dataset.
...
There are some true/false vales represented as 1 and 0. Let's change them.
...
tfa <- tfa %>% 
  mutate(
    ...
    LowIncomeTracts =
      case_when(
        LowIncomeTracts == 0 ~ "No",
        LowIncomeTracts == 1 ~ "Yes"
      )
  )
...
Exports to csv to then join in QGIS.

Also exports as .rds to build maps in R.

```{r}
tfa %>% write_rds("data-processed/tfa.rds")
tfa %>% write_csv("data-processed/tfa.csv")
```
